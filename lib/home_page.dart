...
import 'package:shuffler/blend_initializer.dart';
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<bool> setPlaylistState() async {
    ...
    if (!listEquals(ids, currentIDs)) {
      ...
      await appDB.getAllPlaylists().then(
      ...
      builder:
          (context) => FutureBuilder(
            ...
            builder: (context, snapshot) {
                ...
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  if (snapshot.data as bool) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Center(child: Text('Playlists loaded!')), duration: Duration(seconds: 1)),
                    );
                  }
              ...
              return const AlertDialog(title: Text('Loading playlists...'), content: CircularProgressIndicator());
            },
          ),
    );
    return Future.value();
      ...
      await showDialog(
        ...
        builder:
            (context) => ThemeDialog(
              ...
              setColorScheme: (c) => setState(() => colorScheme = c),
            ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text("Select or Add a Playlist", style: TextStyle(color: colorScheme?.onPrimary)),
        actions: <Widget>[
          PopupMenuButton(
            icon: const Icon(Icons.more_vert),
            itemBuilder:
                (context) => <PopupMenuEntry<String>>[
                  const PopupMenuItem<String>(value: 'Change Theme', child: Text('Change Theme')),
                  const PopupMenuItem<String>(value: 'Nuke Database', child: Text('Nuke Database')),
                  const PopupMenuItem<String>(value: 'Reset Authentication', child: Text('Reset Authentication')),
            ...
            children: <Widget>[
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder:
                      (context, index) => PlaylistDisplayCard(
        ...
        children: [
          FloatingActionButton(
            onPressed:
                () => Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => BlendInitializer(playlists: playlists)),
                ),
